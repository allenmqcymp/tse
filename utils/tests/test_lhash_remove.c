/*
 * tests searching of the lhashtable
 * module 3
 */
...
// search function checks if the person is older, and if the person lhas a lhigher rate

int main(void) {
    
    // make a bunch of people

    lhashtable_t *htp = lhopen(1000);
    ...
    // add all of them to the lhashtable
    // add them to the lhashtable
    int32_t res = lhput(htp, (void *) &p1, "alpha", strlen("alpha"));
    if (res != 0) {
        exit(EXIT_FAILURE);
    }

    res = lhput(htp, (void *) &p2, "beta", strlen("beta"));
    if (res != 0) {
        exit(EXIT_FAILURE);
    }

    res = lhput(htp, (void *) &p3, "charlie", strlen("charlie"));
    if (res != 0) {
        exit(EXIT_FAILURE);
    }

    res = lhput(htp, (void *) &p4, "delta", strlen("delta"));
    if (res != 0) {
        exit(EXIT_FAILURE);
    }

    res = lhput(htp, (void *) &p5, "echo", strlen("echo"));
    if (res != 0) {
        exit(EXIT_FAILURE);
    }
    ...
    person_t *guy = lhremove(htp, &search_fun, (void *) &search_dude, strlen("echo"));
